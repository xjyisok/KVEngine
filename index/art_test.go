...
func TestAdaptiveRadixTree_Put(t *testing.T) {
	art := NewART()
	res1, _ := art.Put([]byte("key-1"), &data.LogRecordPos{Fid: 1, Offset: 12})
	assert.Nil(t, res1)
	art.Put([]byte("key-2"), &data.LogRecordPos{Fid: 1, Offset: 12})
	art.Put([]byte("key-3"), &data.LogRecordPos{Fid: 1, Offset: 12})

	res2, _ := art.Put([]byte("key-3"), &data.LogRecordPos{Fid: 22, Offset: 12})
...
func TestAdaptiveRadixTree_Get(t *testing.T) {
	...
	pos, _ := art.Get([]byte("key-1"))
	assert.NotNil(t, pos)

	pos1, _ := art.Get([]byte("not exist"))
	assert.Nil(t, pos1)

	art.Put([]byte("key-1"), &data.LogRecordPos{Fid: 1123, Offset: 990})
	pos2, _ := art.Get([]byte("key-1"))
...
func TestAdaptiveRadixTree_Size(t *testing.T) {
	...
	art.Put([]byte("key-2"), &data.LogRecordPos{Fid: 1, Offset: 12})
	art.Put([]byte("key-1"), &data.LogRecordPos{Fid: 1, Offset: 12})
	assert.Equal(t, int64(2), art.Size())
}
